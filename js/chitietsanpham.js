...
function addChiTietToWeb(p) {
    ...
    var s = addThongSo('Xuất sứ', p.Xuatsu);
    s += addThongSo('Khối lượng', p.Khoiluong);
    s += addThongSo('Mô tả', p.Mota);
    info.innerHTML = s;
...
function getDetailPromo(sp) {
    switch (sp.KM.LoaiKM) {
        ...
        case 'moiramat':
            return `Khách hàng sẽ dùng được thử mới sản phẩm miễn phí tại cửa hàng`;

        case 'giareonline':
            var del = Number(p.DonGia) - Number(p.KM.GiaTriKM);
            var span = `<span style="font-weight: bold">` + numToString(del) + `</span>`;
            return `Sản phẩm sẽ được giảm ` + span + `₫ khi mua hàng online bằng thẻ ngân hàng`;

        default:
            var span = `<span style="font-weight: bold">1 thẻ giảm giá</span>`;
            return `Cơ hội trúng ` + span + ` khi mua hàng online`;
    }
}
