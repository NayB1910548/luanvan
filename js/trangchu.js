...
window.onload = function() {
    ...
    $("#demoSlider").ionRangeSlider({
        ...
        onFinish: function(data) {
            filtersAjax(createFilters('price', data.from * 1E3 + "-" + data.to * 1E3));
        },
    });
    // ==================== End ===========================
    ...
    // Thêm chọn mức giá
    addPricesRange(0, 100000);
    addPricesRange(100000, 200000);
    addPricesRange(200000, 300000);
    addPricesRange(300000, 400000);
    addPricesRange(400000, 0);
...
function hienThiKhungSanPhamMacDinh() {
    ...
    addKhungSanPham('GIÁ RẺ DƯỚI 100K', green, ['price=0-100000', 'sort=DonGia-asc', 'page=0'], soLuong);
}
        ...
        for (var f of filters) {
            ...
            switch (type) {
                case 'company':
                    ...
                    addChoosedFilter('company', "Loại " + tenHang);
                    break;
...
function priceToString(min, max) {
    if (min == 0) return 'Dưới ' + max / 1E3 + ' trăm';
    if (max == 0) return 'Trên ' + min / 1E3 + ' trăm';
    return 'Từ ' + min / 1E3 + ' - ' + max / 1E3 + ' trăm';
}
